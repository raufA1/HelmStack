...
echo "### 📊 Recent Activity" >> "$MEMORY_FILE"
if [ -f "SESSION_LOG.md" ]; then
    echo "  **Session Activity:**" >> "$MEMORY_FILE"
    tail -5 "SESSION_LOG.md" | sed 's/^/    /' >> "$MEMORY_FILE" 2>/dev/null || echo "    (no recent session activity)" >> "$MEMORY_FILE"
else
    echo "  **Git Activity:**" >> "$MEMORY_FILE"
...
echo "### 🎯 Key Insights" >> "$MEMORY_FILE"
echo "  **Project Health:**" >> "$MEMORY_FILE"
if [ -f "$STATUS_FILE" ]; then
    COMPLETED_TASKS=$(grep -c '^\- \[x\]' "$NEXT_FILE" 2>/dev/null || echo "0")
    PENDING_TASKS=$(grep -c '^\- \[ \]' "$NEXT_FILE" 2>/dev/null || echo "0")
    echo "  - Completed: $COMPLETED_TASKS tasks, Pending: $PENDING_TASKS tasks" >> "$MEMORY_FILE"
else
    echo "  - Run \`hs fix\` to analyze project status" >> "$MEMORY_FILE"
fi

echo "" >> "$MEMORY_FILE"
echo "---" >> "$MEMORY_FILE"
echo "*Memory automatically generated by HelmStack AI system @ $TIMESTAMP*" >> "$MEMORY_FILE"
...
$(if [ -f "$FOCUS_FILE" ]; then head -10 "$FOCUS_FILE"; else echo "Run \`hs fix\` to generate focus list"; fi)
